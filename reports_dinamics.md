...
dinàmics, la veritat es que no sabia l'existència d'aquestes ordres,
pot ser no tenen molt que veure però es lo mes assemblant que he trobat.
...
sorgeix algun problema i ho vols veure gràficament ja ho tens tot en una
mateixa pagina. I per ultim, també he vist que pot ser __multi-server__,
es a dir, que podem supervisar i administrar diversos servidors al mateix
temps.

Peró jo només em centraré en els serveis i veure totes les utilitats que
tenen relació amb aquest tema.

### Com funciona cockpit

Lo primer de tot serà instal·lar el paquet anomenat `cockpit`.

```# dnf -y install cockpit```

Com podem observar, es un servei, així que caldrà iniciar aquest **servei**.
...
nostre ordinador i gràfics que mostren:
...
A l'esquerra tenim una serie de finestres que passo a continuació a explicar:
...
* **Services**
	...
	Aquesta es la part que aprofundiré, al fer clic a qualsevol servei, ens porta a una pàgina de detallada que mostra un registre del servei i que ens permet:
  * Start/Stop
  * Restart
  * Enable/Disable
  * Reload
...
* **Tools**
	...
	Aquest menú es desplega per oferir dues eines de gestió de servidors i usuaris adicionals:
  * **Accounts**: Permet afegir i administrar usuaris, configurar i canviar les contrasenyes, afegir i administrar les claus SSH públiques per a cada usuari (**Òbviament sempre que siguis root**).
  * **Terminal**: Conté un terminal completament funcional, amb l'implementació del tabulador, això ens permet realitzar qualsevol tasca que vulguem.

  ![finestres](images/cockpit/finestres.png)

Una vegada explicat la primera part, passo a explicar la segona finestra que tenim
anomenada __Dashboard__. En aquesta finestra lo que trobem es una gràfica
amb **4 finestretes**, que es per veure els mateixos gràfics que ens surten
...
Per afegir una nova maquina veiem el símbol **+** de color blau que
...
visualitzar-la (molt útil per que si posem el mateix nom en dues maquines
...
Òbviament li donem a `connect` i ja tenim una maquina més i al 
...
Per editar una maquina actual veiem el símbol del `tick`, quan li donem
ens apareixen dos símbols:
...
* **Un llapis**
  ...
  * Avatar
   ...
   Per exemple, canviar el nom i el color com veiem a continuació:
...
que tenim actius, desactivats, engegats, aturats, etc. Peró també tenim
mes apartats dins de services:

* Targets

	Ens mostra tots els targets que tenim en el sistema, es a dir, els que tenim enabled, disabled i statics. Vaig provar caminar d'un runlevel a un altre, clicant per exemple al target ``rescue.target`` i clicant a **start**, però sempre que ho faig es queda el sistema penjat i tinc que reiniciar manualment.
	
* System services

	Aquesta es la part on podem iniciar i aturar serveis, es molt útil aquesta part ja que també podem veure els que estan enabled, disabled i estàtics, també podem veure els logs que es creen automàticament, així sabríem si ens ha fallat alguna cosa o tot funciona be.
	
* Sockets

	Ídem que els anteriors, ens mostren tots els sockets i podem iniciar o aturar el que nosaltres vulguem
	
* Timer

	Aquest apartat conté tots els `.timer`, son temporitzadors que li diuen al servei quan es te que executar.
...
i també a sota veure els logs que ens dona actualment.

![service-httpd-cockpit](images/cockpit/service-httpd.png)

Com podem comprovar, ara mateix esta el servei engegat, ara cliquem
a **stop** i podem veure com l'apartat `Service Logs` s'actualitza
automàticament dient que s'aturat el servei
...
En el moment que ho provava veia que em deia que no trobava coincidències,
llavors, explorant per webs he trobat com es pot instal·lar, cal descarregar-se
un paquet exclusiu per **Fedora 24** (també he trobat per la versió 22 i 23 però
...
Cal dir que si no som root, també s'executa, però en el moment que
...
"Interactive authentication required."` que bàsicament ens diu que
...
els que estan enabled, disabled i statics, veure els logs que es creen 
automàticament, així sabríem si ens ha fallat alguna cosa o tot funciona
be i també poder editar el fitxer de configuració, que la veritat que ho
veig molt útil Trobem al costat del serveis dues rodones:
...
* Enabled/Disabled
	...
	Nomes clicant en aquest icona podem fer el servei **enable** o **disable**

	Com podem veure, el servei `sshd` ara mateix esta **enable**, en el moment
	que cliquem automàticament es posa disable.
...
* Start/Stop
	...
	Si el servei esta engegat, ens sortira l'icona vermell i posarà **Stop**, si cliquem llavors l'icona passara a ser de color blau i posara **Start**

	![service-sshd-start](images/systemd-manager/service-sshd-start.png)

	Com podem comprovar, ara mateix el servei esta engegat, així que ara ens
	...
	ens dona la informació de que el servei `sshd` s'ha aturat i veiem que
